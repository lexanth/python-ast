...
export const grammerFiles = ['Python3.g4'];
    ...
    await Promise.all(
      grammerFiles.map(async (file) => {
        const res = await fetch(
          `https://raw.githubusercontent.com/antlr/grammars-v4/${heads[file]}/python/python3-ts/${file}`,
        );
...
const writeParser = () => {
  ...
  return execCommand(
    `${binary} -visitor -o ${grammerRoot} -Xexact-output-dir ${grammerRoot}/Python3.g4`,
  );
};

const writeParserContexts = async () => {
  const listenerSource = await fs.readFile(
    path.join(__dirname, grammerRoot, 'Python3Listener.ts'),
    'utf-8',
  );
  ...
  await fs.writeFile(
    path.join(__dirname, grammerRoot, 'Python3Contexts.ts'),
    `export {${EOL}${exportList}${EOL}} from './Python3Parser';`,
  );
};
